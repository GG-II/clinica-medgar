    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p class="text-gray-600 mt-1">Registrar un nuevo paciente en el sistema</p>
        </div>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="md:col-span-2">
              ...
              <input
                ...
                type="text"
                class="input-field"
                placeholder="Ej: Agricultor, Comerciante, Ama de casa"
              />
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Número IGSS -->
            <div v-if="form.tiene_igss" class="md:col-span-2">
              <label class="block text-sm font-medium text-gray-700 mb-1">
                Número IGSS
              </label>
              <input
...
async function handleSubmit() {
  ...
  try {
    ...
    const datosBackend = { ...form.value }
    
    // Construir observaciones con campos adicionales
    const observacionesExtras = []
    
    if (form.value.ocupacion) {
      observacionesExtras.push(`Ocupación: ${form.value.ocupacion}`)
    }
    if (form.value.tipo_sangre) {
      observacionesExtras.push(`Tipo de sangre: ${form.value.tipo_sangre}`)
    }
    if (form.value.alergias) {
      observacionesExtras.push(`Alergias: ${form.value.alergias}`)
    }
    if (form.value.condiciones_medicas) {
      observacionesExtras.push(`Condiciones médicas: ${form.value.condiciones_medicas}`)
    }
    
    // Agregar a observaciones
    if (observacionesExtras.length > 0) {
      datosBackend.observaciones = observacionesExtras.join('\n')
    }
    
    // Eliminar campos que el backend no soporta
    delete datosBackend.ocupacion
    delete datosBackend.tipo_sangre
    delete datosBackend.alergias
    delete datosBackend.condiciones_medicas
    
    await crearPaciente(datosBackend)
    // El composable redirige automáticamente al detalle
  } catch (error) {
    errorGeneral.value = error.message || 'Error al crear el paciente'
  }
}
